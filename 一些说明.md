...
```sql
SELECT setval(
    pg_get_serial_sequence('AttributeKey', 'attr_id'),
    (SELECT COALESCE(MAX(attr_id), 1) FROM AttributeKey)
);

SELECT pg_get_serial_sequence('AttributeKey', 'attr_id');
